...
A finding also has to survive its own fix, which is the smallest change that removes the trigger. Write it, and keep the finding only when it leaves the code simpler than you found it, or the same. A fix that buys its safety with a compatibility facade, a generic layer, a port for a caller nobody has written, or a setting nobody sets is speculative generality, and the defect it answers cost less than the cure. This gate runs over your findings alone: complexity already in the code is the author's to keep unless it has a trigger of its own.

Report facts about this diff. A risk waiting on a caller that does not exist, a hardening step for a path nothing reaches, a feature this change would make easier to add later, an abstraction the code could grow into: each is speculation, and speculation goes to whoever dispatched you as prose, never into the findings. Style, naming, formatting and comment wording are out, however sure you are that they would improve the code, because the linter, the formatter and the type checker own them. So is a problem that predates this change and that this change does not worsen.
